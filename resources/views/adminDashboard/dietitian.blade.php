...
@section('content')
    ...
    <div class="w-full max-w-xs md:max-w-7xl p-8 bg-white rounded-lg my-4 text-center shadow-md mx-auto animate-fade-in" x-data>
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <h2 class="text-xl sm:text-2xl font-bold">Dietitian Management Summary</h2>

            <div class="flex justify-between items-center space-x-3 sm:space-x-4">
                <div class="relative sm:w-auto">
                    <form method="GET" action="{{ route('admin.dietitian') }}">
                        <input
                            type="text"
                            id="searchBar"
                            name="search"
                ...
                <div class="relative group">
                    <form method="POST" action="{{ route('user.report') }}" target="_blank" class="relative group" onsubmit="setCurrentDateTime()">
                        ...
                        <input type="hidden" name="role" value="Dietitian">
        ...
        <!-- Action Buttons -->
        <form method="POST" action="{{ route('dietitian.bulkAction') }}">
            @csrf
                        ...
                        @foreach ($dietitians as $dietitian)
                            <tr class="hover:bg-gray-200 transition duration-200">
                                <td class="py-3 px-4 text-left border-b border-gray-200">
                                    <input type="checkbox" name="selector[]" class="h-4 w-4" value="{{ $dietitian->id }}">
                                </td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $dietitian->first_name }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $dietitian->last_name }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $dietitian->email }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $dietitian->mobile_number }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $dietitian->address }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">
                                    <span class="{{ $dietitian->roles->first()?->pivot->is_active ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold' }}">
                                        {{ $dietitian->roles->first()?->pivot->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">
                                    <a href="javascript:void(0);" onclick="openEditModal(this)" data-id="{{ $dietitian->id }}" data-first_name="{{ $dietitian->first_name }}"
                                        data-last_name="{{ $dietitian->last_name }}" data-email="{{ $dietitian->email }}" data-contact_number="{{ $dietitian->mobile_number }}"
        ...
        <!-- Modal -->
        <div id="addDietitianModal" role="dialog" aria-modal="true" class="fixed inset-0 backdrop-blur-sm bg-white/20 hidden items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 w-full max-w-md shadow-[0_0_15px_4px_rgba(241,30,19,0.5)]">
                <h2 class="text-md md:text-3xl font-bold text-center mb-4" id="modalHeader">Create dietitian Account</h2>                

                <form method="POST" id="dietitianForm">
                    @csrf
                    <input type="hidden" id="dietitian_id" name="dietitian_id">
                    <div>
    ...
    @push('scripts')
        ...
        <script>
        document.querySelector('form[action="{{ route('dietitian.bulkAction') }}"]').addEventListener('submit', function(e) {
        ...
        function openModal() {
            ...
            document.getElementById('dietitianForm').action = "{{ route('dietitian.create') }}";
            document.getElementById('modalSubmitButton').textContent = 'Create';
            document.getElementById('modalHeader').textContent = 'Create Dietitian Account';
            const modal = document.getElementById('addDietitianModal');
            if (modal) {
        ...
        function openEditModal(button) {
            ...
            document.getElementById('dietitian_id').value = id;
            ...
            form.action = "{{ route('dietitian.update') }}"; 

            document.getElementById('modalSubmitButton').textContent = 'Update';
            document.getElementById('modalHeader').textContent = 'Update Dietitian Account';
            const modal = document.getElementById('addDietitianModal');
            if (modal) {
                modal.classList.remove('hidden');
                modal.classList.add('flex');

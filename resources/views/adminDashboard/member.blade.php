...
@section('content')
    ...
    <div class="w-full max-w-xs md:max-w-7xl p-5 bg-white rounded-lg my-4 text-center shadow-md mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <h2 class="text-xl sm:text-2xl font-bold">Member Management Summary</h2>

            <div class="flex justify-between items-center space-x-3 sm:space-x-4">
                <div class="relative sm:w-auto">
                    <form method="GET" action="{{ route('admin.member') }}">
                        <input
                            type="text"
                            id="searchBar"
                            name="search"
        ...
        <!-- Action Buttons -->
        <form method="POST" action="{{ route('member.bulkAction') }}">
            @csrf
                        ...
                        @foreach ($members as $member)
                            <tr class="hover:bg-gray-200 transition duration-200">
                                <td class="py-3 px-4 text-left border-b border-gray-200">
                                    <input type="checkbox" name="selector[]" class="h-4 w-4" value="{{ $member->id }}">
                                </td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $member->first_name }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $member->last_name }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $member->email }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $member->mobile_number }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $member->address }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">
                                    <span class="{{ $member->roles->first()?->pivot->is_active ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold' }}">
                                        {{ $member->roles->first()?->pivot->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">
                                    <a href="javascript:void(0);" onclick="openEditModal(this)" data-id="{{ $member->id }}" data-first_name="{{ $member->first_name }}"
                                        data-last_name="{{ $member->last_name }}" data-email="{{ $member->email }}" data-contact_number="{{ $member->mobile_number }}"
        ...
        <!-- Modal -->
        <div id="addMemberModel" role="dialog" aria-modal="true" class="fixed inset-0 backdrop-blur-sm bg-white/20 hidden items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 w-full max-w-md shadow-[0_0_15px_4px_rgba(241,30,19,0.5)]">
                <h2 class="text-md md:text-3xl font-bold text-center mb-4" id="modalHeader">Create Member Account</h2>                
                ...
                <form method="POST" id="memberForm">
                    ...
                    <div class="flex justify-end space-x-2 mt-4">
                        <button type="button" class="bg-gray-300 hover:bg-gray-400 text-black px-4 py-2 rounded"><a href="{{ route('admin.member') }}">Cancel</a></button>
    ...
    @push('scripts')
        ...
        <script>
        document.querySelector('form[action="{{ route('member.bulkAction') }}"]').addEventListener('submit', function(e) {
        ...
        function openModal() {
            ...
            document.getElementById('memberForm').action = "{{ route('member.create') }}";
            document.getElementById('modalSubmitButton').textContent = 'Create';
            document.getElementById('modalHeader').textContent = 'Create Member Account';
            const modal = document.getElementById('addMemberModel');
            if (modal) {
        ...
        function openEditModal(button) {
            ...
            document.getElementById('modalHeader').textContent = 'Update Member Account';
            const modal = document.getElementById('addMemberModel');
            if (modal) {
                modal.classList.remove('hidden');
                modal.classList.add('flex');

...
@section('content')
    ...
    <div class="w-full max-w-xs md:max-w-7xl p-5 bg-white rounded-lg my-4 text-center shadow-md mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <h2 class="text-xl sm:text-2xl font-bold">Trainer Management Summary</h2>

            <div class="flex justify-between items-center space-x-3 sm:space-x-4">
                <div class="relative sm:w-auto">
                    <form method="GET" action="{{ route('admin.trainer') }}">
                        <input
                            type="text"
                            id="searchBar"
                            name="search"
        ...
        <!-- Action Buttons -->
        <form method="POST" action="{{ route('trainer.bulkAction') }}">
            @csrf
                        ...
                        @foreach ($trainers as $trainer)
                            <tr class="hover:bg-gray-200 transition duration-200">
                                <td class="py-3 px-4 text-left border-b border-gray-200">
                                    <input type="checkbox" name="selector[]" class="h-4 w-4" value="{{ $trainer->id }}">
                                </td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $trainer->first_name }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $trainer->last_name }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $trainer->email }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $trainer->mobile_number }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">{{ $trainer->address }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">
                                    <span class="{{ $trainer->roles->first()?->pivot->is_active ? 'text-green-600 font-semibold' : 'text-red-600 font-semibold' }}">
                                        {{ $trainer->roles->first()?->pivot->is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">
                                    <a href="javascript:void(0);" onclick="openEditModal(this)" data-id="{{ $trainer->id }}" data-first_name="{{ $trainer->first_name }}"
                                        data-last_name="{{ $trainer->last_name }}" data-email="{{ $trainer->email }}" data-contact_number="{{ $trainer->mobile_number }}"
        ...
        <!-- Modal -->
        <div id="addTrainerModal" role="dialog" aria-modal="true" class="fixed inset-0 backdrop-blur-sm bg-white/20 hidden items-center justify-center z-50">
            <div class="bg-white rounded-lg p-6 w-full max-w-md shadow-[0_0_15px_4px_rgba(241,30,19,0.5)]">
                <h2 class="text-md md:text-3xl font-bold text-center mb-4" id="modalHeader">Create Trainer Account</h2>                
    ...
    @push('scripts')
        ...
        document.querySelector('form[action="{{ route('trainer.bulkAction') }}"]').addEventListener('submit', function(e) {
            ...
            if (checkboxes.length === 0) {
                ...
                Swal.fire({
                    icon: 'warning',
                    title: 'No selection',
                    text: 'Please select at least one trainer.',
                    confirmButtonColor: '#d32f2f'
                });
                return; 
            }
        ...
        function openModal() {
            ...
            document.getElementById('trainerForm').action = "{{ route('trainer.create') }}";
            document.getElementById('modalSubmitButton').textContent = 'Create';
            document.getElementById('modalHeader').textContent = 'Create Trainer Account';
        ...
        function openEditModal(button) {
            ...
            form.action = "{{ route('trainer.update') }}"; 

            document.getElementById('modalSubmitButton').textContent = 'Update';
            document.getElementById('modalHeader').textContent = 'Update Trainer Account';
            const modal = document.getElementById('addTrainerModal');
            if (modal) {
                modal.classList.remove('hidden');
                modal.classList.add('flex');

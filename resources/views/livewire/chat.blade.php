<div class="w-full max-w-xs md:max-w-7xl p-8 bg-white rounded-lg my-4 text-center shadow-md mx-auto animate-fade-in" x-data>
    <div class="flex h-[600px] border rounded-2xl shadow-sm overflow-hidden bg-white">
        ...
        <div class="w-2/3 flex flex-col bg-white">
            ...
            @else
                <div class="p-6 text-center select-none border-b border-gray-200 
                ...
                @foreach($messages as $message)
                    <div wire:poll.60s
                        ...
                        @if($message->sender_id === Auth::id())
                            oncontextmenu="event.preventDefault(); if(confirm('Delete this message?')) { @this.deleteMessage({{ $message->id }}); }"
            ...
            <!-- Input -->
            <form wire:submit.prevent="sendMessage" class="p-5 border-t border-gray-200 bg-white flex items-center gap-4">
                <input
                ...
                <button
                    type="submit"
                    class="bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-6 py-3 rounded-full shadow-md transition"
                >
                    Send
                </button>
            </form>

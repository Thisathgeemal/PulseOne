...
$emptyIcon = $isTrainer ? 'M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z' : 'M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z';
$emptyTitle = $isTrainer ? 'No feedback yet' : 'No feedback yet';
$emptyDesc = $isTrainer ? 'You haven\'t received any feedback from members yet. Keep up the great work and feedback will start coming in!' : 'You haven\'t received any feedback from members yet. Keep providing excellent guidance and feedback will start coming in!';
@endphp
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-bold text-gray-900">{{ number_format($items->where('rate', '!=', null)->avg('rate'), 1) }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 border border-gray-100">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="text-2xl font-bold text-gray-900">{{ $items->where('created_at', '>=', now()->startOfMonth())->count() }}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100 overflow-hidden">
            ...
            <div class="bg-gradient-to-r {{ $cardGradient }} p-4">
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 class="text-white font-semibold text-lg">{{ $fb->fromUser->first_name }} {{ $fb->fromUser->last_name }}</h3>
                    <p class="{{ $textColor }} text-sm">{{ \Carbon\Carbon::parse($fb->created_at)->format('M d, Y') }}</p>
                  </div>
                </div>
            ...
            <div class="p-6">
              ...
              <div class="flex items-center justify-between pt-4 mt-4 border-t border-gray-100">
                ...
                <span class="text-gray-500 text-sm">{{ \Carbon\Carbon::parse($fb->created_at)->diffForHumans() }}</span>
              </div>
            </div>
          </div>
        @endforeach

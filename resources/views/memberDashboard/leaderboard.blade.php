...
@section('content')
    ...
    <div class="p-6">
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div
                class="inline-flex bg-gray-100 dark:bg-gray-800 rounded-full p-1 shadow border border-gray-200 dark:border-gray-700">
        ...
        @if (empty($entries))
            <div class="no-data rounded-2xl p-12 text-center bg-white dark:bg-gray-900 shadow">
                ...
                <p class="text-lg text-gray-600 dark:text-gray-300">Kick things off by logging workouts, attendance, and
                    meals to appear here.</p>
            </div>
        @else
            <div class="max-w-full">
                ...
                <div class="lb-champion-wrapper" style="position: relative;">
                    ...
                    <div class="lb-title"
                        style="background:#000; color:#fff; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:4px; padding:12px 20px; min-height:72px;">
                        ...
                        <span class="text-xs sm:text-sm" style="opacity:0.92;">
                            {{ isset($start) ? $start->format('M d, Y') : '' }} —
                            {{ isset($end) ? $end->format('M d, Y') : '' }}
                        </span>
                    </div>
                                    ...
                                    <div class="relative inline-block">
                                        <div class="lb-podium-avatar lb-ring-2 rounded-full bg-white relative mx-auto">
                                            @if (!empty($second['user']->profile_image))
                                                <img src="{{ asset($second['user']->profile_image) }}?v={{ time() }}"
                                                    class="w-full h-full object-cover"
                                                    alt="{{ $second['user']->first_name }}">
                                            @else
                                                <div
                                                    class="w-full h-full flex items-center justify-center text-3xl font-bold bg-gradient-to-br from-gray-100 to-gray-200 text-red-600">
                                                    {{ $initials($second['user']) }}</div>
                                            @endif
                                        </div>
                                        <div class="lb-podium-badge lb-podium-badge-2"
                                            style="background:transparent; box-shadow:none; width:auto; height:auto;">
                                            <img src="{{ asset('images/silver.png') }}" alt="Silver"
                                                class="lb-medal-silver">
                                        </div>
                                    </div>
                                    <div class="mt-4">
                                        <div class="text-2xl font-bold mt-6 mb-2">
                                            {{ $second['user']->first_name ?? 'Member' }}</div>
                                        ...
                                        <div class="mt-3 lb-small-stats text-center">
                                            <div class="lb-score-display lb-score-display-sm inline-block mb-2">
                                                <div class="text-xl font-bold">
                                                    {{ number_format($second['score_display'] ?? $second['score'], 3) }}
                                                </div>
                                                <div class="text-xs opacity-80">Score</div>
                                            </div>
                                            <div class="text-xs opacity-80 mb-1">Attendances <span
                                                    class="font-bold">{{ round($second['attendance_pct'] ?? 0, 1) }}%</span>
                                            </div>
                                            <div class="lb-progress-bar lb-progress-small mb-2">
                                                <div class="lb-progress-fill"
                                                    style="width: {{ min(100, max(0, round($second['attendance_pct'] ?? 0, 1))) }}%">
                                                </div>
                                            </div>
                                            <div class="text-xs opacity-80 mb-1">Workout <span
                                                    class="font-bold">{{ isset($second['workout_pct']) ? number_format($second['workout_pct'], 1) . '%' : (isset($second['workout_days']) ? number_format($second['workout_days'], 1) . '%' : '0%') }}</span>
                                            </div>
                                            <div class="lb-progress-bar lb-progress-small mb-2">
                                                <div class="lb-progress-fill"
                                                    style="width: {{ min(100, max(0, round($second['workout_pct'] ?? ($second['workout_days'] ?? 0), 1))) }}%">
                                                </div>
                                            </div>
                                            <div class="text-xs opacity-80 mb-1">Diet <span
                                                    class="font-bold">{{ isset($second['compliance_pct']) ? number_format($second['compliance_pct'], 1) . '%' : '0%' }}</span>
                                            </div>
                                            <div class="lb-progress-bar lb-progress-small">
                                                <div class="lb-progress-fill"
                                                    style="width: {{ min(100, max(0, round($second['diet_pct'] ?? ($second['compliance_pct'] ?? 0), 1))) }}%">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="text-center px-4 relative">
                                @if ($top)
                                    <div class="relative inline-block text-center">
                                        ...
                                        <div
                                            class="lb-podium-avatar lb-champion-avatar lb-ring-1 rounded-full bg-white relative mx-auto">
                                            @if (!empty($top['user']->profile_image))
                                                <img src="{{ asset($top['user']->profile_image) }}?v={{ time() }}"
                                                    class="w-full h-full object-cover"
                                                    alt="{{ $top['user']->first_name }}">
                                            @else
                                                <div
                                                    class="w-full h-full flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 text-5xl font-extrabold text-red-600">
                                                    {{ $initials($top['user']) }}</div>
                                            @endif
                                        </div>
                                        <div class="lb-podium-badge lb-podium-badge-1"
                                            style="background:transparent; box-shadow:none; width:auto; height:auto;">
                                    ...
                                    <div class="mt-8">
                                        <h2 class="text-4xl font-extrabold mb-2">{{ $top['user']->first_name ?? 'Member' }}
                                            {{ $top['user']->last_name ?? '' }}</h2>
                                        <div class="lb-score-display inline-block">
                                            <div class="text-3xl font-bold">
                                                {{ number_format($top['score_display'] ?? $top['score'], 3) }}</div>
                                    ...
                                    <div class="mt-6 lb-champion-compact-stats mx-auto text-center">
                                        <div class="lb-small-stats lb-champion-small">
                                            <div class="text-sm opacity-80 mb-1">Attendances <span
                                                    class="font-bold">{{ round($top['attendance_pct'] ?? 0, 1) }}%</span>
                                            </div>
                                            <div class="lb-progress-bar lb-progress-large mb-3">
                                                <div class="lb-progress-fill"
                                                    style="width: {{ min(100, max(0, round($top['attendance_pct'] ?? 0, 1))) }}%">
                                                </div>
                                            </div>

                                            <div class="text-sm opacity-80 mb-1">Workout <span
                                                    class="font-bold">{{ isset($top['workout_pct']) ? number_format($top['workout_pct'], 1) . '%' : (isset($top['workout_days']) ? number_format($top['workout_days'], 1) . '%' : '0%') }}</span>
                                            </div>
                                            <div class="lb-progress-bar lb-progress-large mb-3">
                                                <div class="lb-progress-fill"
                                                    style="width: {{ min(100, max(0, round($top['workout_pct'] ?? ($top['workout_days'] ?? 0), 1))) }}%">
                                                </div>
                                            </div>

                                            <div class="text-sm opacity-80 mb-1">Diet <span
                                                    class="font-bold">{{ isset($top['compliance_pct']) ? number_format($top['compliance_pct'], 1) . '%' : '0%' }}</span>
                                            </div>
                                            <div class="lb-progress-bar lb-progress-large">
                                                <div class="lb-progress-fill"
                                                    style="width: {{ min(100, max(0, round($top['diet_pct'] ?? ($top['compliance_pct'] ?? 0), 1))) }}%">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="relative inline-block">
                                        <div class="lb-podium-avatar lb-ring-3 rounded-full bg-white relative mx-auto">
                                            @if (!empty($third['user']->profile_image))
                                                <img src="{{ asset($third['user']->profile_image) }}?v={{ time() }}"
                                                    class="w-full h-full object-cover"
                                                    alt="{{ $third['user']->first_name }}">
                                            @else
                                                <div
                                                    class="w-full h-full flex items-center justify-center text-3xl font-bold bg-gradient-to-br from-gray-100 to-gray-200 text-red-600">
                                                    {{ $initials($third['user']) }}</div>
                                            @endif
                                        </div>
                                        <div class="lb-podium-badge lb-podium-badge-3"
                                            style="background:transparent; box-shadow:none; width:auto; height:auto;">
                                            <img src="{{ asset('images/bronze.png') }}" alt="Bronze"
                                                class="lb-medal-bronze">
                                        </div>
                                    </div>
                                    <div class="mt-4">
                                        <div class="text-2xl font-bold mt-6 mb-2">
                                            {{ $third['user']->first_name ?? 'Member' }}</div>
                                        {{-- plain pts line removed; show boxed score below instead --}}
                                        {{-- compact stats under 3rd place --}}
                                        <div class="mt-3 lb-small-stats text-center">
                                            <div class="lb-score-display lb-score-display-sm inline-block mb-2">
                                                <div class="text-xl font-bold">
                                                    {{ number_format($third['score_display'] ?? $third['score'], 3) }}</div>
                                                <div class="text-xs opacity-80">Score</div>
                                            </div>
                                            <div class="text-xs opacity-80 mb-1">Attendances <span
                                                    class="font-bold">{{ round($third['attendance_pct'] ?? 0, 1) }}%</span>
                                            </div>
                                            <div class="lb-progress-bar lb-progress-small mb-2">
                                                <div class="lb-progress-fill"
                                                    style="width: {{ min(100, max(0, round($third['attendance_pct'] ?? 0, 1))) }}%">
                                                </div>
                                            </div>
                                            <div class="text-xs opacity-80 mb-1">Workout <span
                                                    class="font-bold">{{ isset($third['workout_pct']) ? number_format($third['workout_pct'], 1) . '%' : (isset($third['workout_days']) ? number_format($third['workout_days'], 1) . '%' : '0%') }}</span>
                                            </div>
                                            <div class="lb-progress-bar lb-progress-small mb-2">
                                                <div class="lb-progress-fill"
                                                    style="width: {{ min(100, max(0, round($third['workout_pct'] ?? ($third['workout_days'] ?? 0), 1))) }}%">
                                                </div>
                                            </div>
                                            <div class="text-xs opacity-80 mb-1">Diet <span
                                                    class="font-bold">{{ isset($third['compliance_pct']) ? number_format($third['compliance_pct'], 1) . '%' : '0%' }}</span>
                                            </div>
                                            <div class="lb-progress-bar lb-progress-small">
                                                <div class="lb-progress-fill"
                                                    style="width: {{ min(100, max(0, round($third['diet_pct'] ?? ($third['compliance_pct'] ?? 0), 1))) }}%">
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                            ...
                                            <div class="lb-table-grid">
                                                ...
                                                <div>
                                                    <div class="flex items-center space-x-4">
                                                        <div
                                                            class="lb-member-avatar bg-gradient-to-br from-gray-100 to-gray-200">
                                                            @if (!empty($entry['user']->profile_image))
                                                                <img src="{{ asset($entry['user']->profile_image) }}?v={{ time() }}"
                                                                    class="w-full h-full object-cover"
                                                                    alt="{{ $entry['user']->first_name }}">
                                                            @else
                                                                <div
                                                                    class="w-full h-full flex items-center justify-center text-lg font-bold text-red-600">
                                                                    {{ $initials($entry['user']) }}</div>
                                                            @endif
                                                        </div>
                                                        <div>
                                                            <div class="font-bold text-lg"
                                                                style="color: #000000 !important;">
                                                                {{ $entry['user']->first_name ?? 'Member' }}
                                                                {{ $entry['user']->last_name ?? '' }}</div>
                                                ...
                                                <div>
                                                    <div class="lb-stat-value text-xl">
                                                        {{ number_format($entry['score_display'] ?? $entry['score'], 3) }}
                                                    </div>
                                                </div>
                                                <div class="hidden md:block">
                                                    <div class="lb-stat-value">
                                                        {{ round($entry['attendance_pct'] ?? 0, 1) }}%</div>
                                                </div>
                                                <div class="hidden md:block">
                                                    <div class="lb-stat-value">
                                                        {{ isset($entry['workout_pct']) ? number_format($entry['workout_pct'], 1) . '%' : (isset($entry['workout_days']) ? number_format($entry['workout_days'], 1) . '%' : '0%') }}
                                                    </div>
                                                </div>
                                                <div class="hidden md:block">
                                                    <div class="lb-stat-value">
                                                        {{ isset($entry['compliance_pct']) ? number_format($entry['compliance_pct'], 1) . '%' : '0%' }}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>

...
@section('content')
    ...
    <div class="w-full max-w-7xl p-8 bg-white rounded-lg my-4 shadow-md mx-auto" x-data="bookingForm()"
        x-init="init()">
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <button @click="checkWorkoutPlanAndOpenModal()" class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded">
                + New Booking
            </button>
        </div>
                ...
                <tbody>
                    @forelse($bookings as $b)
                        <tr class="hover:bg-gray-100 transition duration-150">
                            ...
                            <td class="py-3 px-4 border-b border-gray-200">
                                @if ($b->status === 'pending')
                                    <form method="POST" action="{{ route('member.bookings.cancel', $b->booking_id) }}"
                                        class="cancel-booking-form">
                                        @csrf
                                        <button type="button"
                                            class="px-3 py-1 rounded bg-red-500 hover:bg-red-600 text-white cancel-btn">
                                            Cancel
                                        </button>
                                    </form>
                                @else
            ...
            function bookingForm() {
                return {
                    ...
                    async loadSlots() {
                        ...
                        try {
                            const url =
                                `{{ route('member.bookings.slots') }}?trainer_id=${encodeURIComponent(t)}&date=${encodeURIComponent(d)}`;

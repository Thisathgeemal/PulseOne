        ...
        <div class="max-w-4xl mx-auto py-8 px-4">
                    ...
                    <div class="flex">
                        ...
                        <div class="ml-3">
                            <p class="text-sm text-yellow-700">
                                <strong>Assessment Update Required:</strong> Your health assessment was completed more than
                                6 months ago
                                ({{ $assessment->completed_at->format('M j, Y') }}). Please update your information to
                                ensure accurate health monitoring and workout planning.
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="mt-4 p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
                        <div class="flex items-center">
                            ...
                            <p class="text-yellow-800">
                                Your health assessment is over 6 months old. Please update it to continue accessing all
                                features.
                            </p>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
                ...
                <div class="w-full bg-gray-200 rounded-full h-2">
                    <div class="accent-bg h-2 rounded-full transition-all duration-500" :style="`width: ${progress}%`">
                    </div>
                </div>
            </div>

            <form method="POST" action="{{ route('member.health-assessment.store') }}" @submit="validateAndSubmit($event)"
                id="healthAssessmentForm" novalidate>
                @csrf
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="height_cm" min="100" max="250" step="0.1"
                                ...
                                value="{{ old('height_cm', $assessment?->height_cm) }}" placeholder="e.g., 175">
                            @error('height_cm')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="weight_kg" min="30" max="300" step="0.1"
                                class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-red-500 focus:border-red-500"
                                value="{{ old('weight_kg', $assessment?->weight_kg) }}" placeholder="e.g., 70">
                            @error('weight_kg')
                                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                            @enderror
                        </div>
            ...
            'extra_active' => 'Extra Active - Very hard exercise, physical job',
                                    ...
                                    <label
                                        ...
                                        <input type="radio" name="activity_level" value="{{ $key }}" required
                                            @checked(old('activity_level', $assessment?->activity_level) == $key) class="text-blue-500 focus:ring-blue-500">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Fitness Goals * (Select all that
                                apply)</label>
            ...
            'rehabilitation' => 'Rehabilitation',
                                    ...
                                    <label
                                        class="flex items-center p-2 border border-gray-300 rounded hover:bg-gray-50 cursor-pointer">
                                        <input type="checkbox" name="fitness_goals[]" value="{{ $key }}"
                                            @checked(in_array($key, old('fitness_goals', $assessment?->fitness_goals ?? []))) class="text-blue-500 focus:ring-blue-500">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div class="space-y-6">
                        ...
                        <div x-data="{
                            ...
                            <label class="block text-sm font-medium text-gray-700 mb-2">Current Medications (if
                                any)</label>
                            <div class="space-y-2">
                                <template x-for="(item, index) in items" :key="index">
                                    <div class="flex gap-2">
                                        <input type="text" x-model="items[index]" :name="`medications[${index}]`"
                        ...
                        <div x-data="{
                            ...
                            <label class="block text-sm font-medium text-gray-700 mb-2">Previous Injuries & Surgeries (if
                                any)</label>
                            <div class="space-y-2">
                                <template x-for="(item, index) in items" :key="index">
                                    <div class="flex gap-2">
            ...
            'pet_dander' => 'Pet Dander',
                                    ...
                                    <label
                                        ...
                                        <input type="checkbox" name="allergies[]" value="{{ $key }}"
                                            @checked(in_array($key, old('allergies', $assessment?->allergies ?? []))) class="text-blue-500 focus:ring-blue-500">
                                        <span class="ml-2 text-sm">{{ $label }}</span>
                                    </label>
                                @endforeach
                            </div>
                            ...
                            @php
                                ...
                                $customAllergies = array_values(
                                    array_filter($userAllergies, function ($allergy) use ($commonAllergies) {
                            ...
                            <div x-data="{
                                ...
                                <div class="space-y-2">
                                    <template x-for="(item, index) in items" :key="index">
                                        <div class="flex gap-2">
                                            <input type="text" x-model="items[index]"
                                                :name="`allergies[${index + 12}]`" placeholder="Specify other allergies..."
                                                class="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-red-500 focus:border-red-500">
                                            <button type="button" @click="removeItem(index)" x-show="items.length > 1"
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-3">Dietary Restrictions (if
                                any)</label>
            ...
            'halal' => 'Halal',
                                    ...
                                    <label
                                        ...
                                        <input type="checkbox" name="dietary_restrictions[]" value="{{ $key }}"
                                            @checked(in_array($key, old('dietary_restrictions', $assessment?->dietary_restrictions ?? []))) class="text-blue-500 focus:ring-blue-500">
                                        <span class="ml-2 text-sm">{{ $label }}</span>
                                    </label>
                                @endforeach
                            </div>
                            ...
                            @php
                                ...
                                $customDietaryRestrictions = array_values(
                                    array_filter($userDietaryRestrictions, function ($restriction) use (
                                        $commonDietaryRestrictions,
                                    ) {
                                        return !in_array($restriction, $commonDietaryRestrictions) &&
                                            !empty(trim($restriction));
                                    }),
                                );
                            @endphp
                            <div x-data="{
                                ...
                                <label class="block text-sm font-medium text-gray-600 mb-2">Other Dietary
                                    Restrictions</label>
                                <div class="space-y-2">
                                    <template x-for="(item, index) in items" :key="index">
                                        <div class="flex gap-2">
                ...
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    @php
                        $defaultContact = [
                            [
                                'name' => $assessment?->emergency_contact_name ?? '',
                                'phone' => $assessment?->emergency_contact_phone ?? '',
                                'relation' => $assessment?->emergency_contact_relation ?? '',
                            ],
                        ];
                        $emergencyContacts = old(
                            'emergency_contacts',
                            $assessment?->emergency_contacts ?? $defaultContact,
                        );
                        // Ensure we always have at least one contact
                        if (
                            empty($emergencyContacts) ||
                            (count($emergencyContacts) == 1 &&
                                empty($emergencyContacts[0]['name']) &&
                                empty($emergencyContacts[0]['phone']) &&
                                empty($emergencyContacts[0]['relation']))
                        ) {
                                ...
                                <div class="grid md:grid-cols-12 gap-4 p-4 border border-gray-200 rounded-lg">
                                    ...
                                    <div class="md:col-span-4">
                                        ...
                                        <input type="tel" x-model="contacts[index].phone"
                                            :name="`emergency_contacts[${index}][phone]`" required
                                    ...
                                    <div class="md:col-span-1 flex items-end">
                                        <button type="button" @click="removeContact(index)" x-show="contacts.length > 1"
                    ...
                    <div class="space-y-4">
                        ...
                        @foreach ($parqQuestions as $index => $question)
                            <div class="p-4 border border-gray-200 rounded-lg">
                                <p class="text-sm font-medium text-gray-900 mb-3">{{ $index + 1 }}.
                                    {{ $question }}</p>
                                <div class="flex gap-6">
                                    <label class="flex items-center">
                                        <input type="radio" name="par_q_responses[{{ $index }}]" value="0"
                                            required @checked(old("par_q_responses.{$index}", $assessment?->par_q_responses[$index] ?? null) === '0')
                                            class="text-green-500 focus:ring-green-500">
                                        <span class="ml-2 text-sm text-green-700">No</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="radio" name="par_q_responses[{{ $index }}]" value="1"
                                            required @checked(old("par_q_responses.{$index}", $assessment?->par_q_responses[$index] ?? null) === '1')
                    ...
                    <div class="space-y-4">
                        <div class="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                            <p class="text-sm text-blue-800">
                                ...
                                If you answered "Yes" to any PAR-Q+ questions above, medical clearance from your doctor is
                                required.
                            </p>
                        </div>
                            ...
                            <div class="space-y-2">
                                <label
                                    ...
                                    <input type="radio" name="doctor_clearance" value="1" required
                                        @checked(old('doctor_clearance', $assessment?->doctor_clearance) === '1' ||
                                                old('doctor_clearance', $assessment?->doctor_clearance) === true) class="text-green-500 focus:ring-green-500">
                                ...
                                <label
                                    ...
                                    <input type="radio" name="doctor_clearance" value="0" required
                                        @checked(old('doctor_clearance', $assessment?->doctor_clearance) === '0' ||
                                                old('doctor_clearance', $assessment?->doctor_clearance) === false) class="text-red-500 focus:ring-red-500">
                ...
                return {
                    ...
                    validateForm() {
                        ...
                        if (!this.getFieldValue('exercise_experience')) {
                            errors.push({
                                field: 'exercise_experience',
                                message: 'Please select your exercise experience level'
                            });
                        }
                        if (!this.getFieldValue('preferred_workout_time')) {
                            errors.push({
                                field: 'preferred_workout_time',
                                message: 'Please select your preferred workout time'
                            });
                        }
                        ...
                        if (!emergencyPhone) {
                            errors.push({
                                field: 'emergency_contacts[0][phone]',
                                message: 'Emergency contact phone is required'
                            });
                        }
                        if (!emergencyRelation) {
                            errors.push({
                                field: 'emergency_contacts[0][relation]',
                        ...
                        if (!this.getFieldValue('smoking_status')) {
                            errors.push({
                                ...
                                message: 'Please select your smoking status'
                            });
                        }
                        if (!this.getFieldValue('alcohol_consumption')) {
                            errors.push({
                                field: 'alcohol_consumption',
                                message: 'Please select your alcohol consumption level'
                            });
                        }
                        ...
                        if (!sleepHours || isNaN(sleepHours) || sleepHours < 1 || sleepHours > 24) {
                            errors.push({
                                field: 'sleep_hours',
                                message: 'Please enter valid sleep hours (1-24)'
                            });
                        }
                        ...
                        if (yesResponses.length > 0 && !doctorClearance) {
                            ...
                            errors.push({
                                field: 'doctor_clearance',
                                message: 'Doctor clearance is required due to your health questionnaire responses'
                            });
                        }
                        ...
                        errors.forEach(error => {
                            ...
                            if (field) {
                                ...
                                if (error.field === 'fitness_goals') {
                                    ...
                                    if (container && !container.nextElementSibling?.classList.contains(
                                            'validation-error')) {
                    ...
                    isFieldGroupComplete(fieldName) {
                        ...
                        switch (fieldName) {
                            ...
                            case 'doctor_clearance':
                                const selectField = form.querySelector(
                                    `select[name="${fieldName}"], input[name="${fieldName}"]:checked`);

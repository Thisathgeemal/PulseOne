...
@section('content')
    ...
    <div class="w-full max-w-xs md:max-w-7xl mx-auto">
        ...
        @forelse($upcoming as $s)
            @php
                ...
                $start = $s->start_at
                    ? \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $s->getRawOriginal('start_at'), 'UTC')->setTimezone('Asia/Colombo')
                    : \Carbon\Carbon::parse(
                        (\Carbon\Carbon::parse($s->date)->toDateString() ?? now()->toDateString()) . ' ' . ($s->time ?? '00:00:00'),
                        'Asia/Colombo'
                      );
            ...
            <div class="group bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-300 p-6 mb-4 overflow-hidden">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <span class="inline-block text-xs font-semibold px-3 py-1 rounded-full bg-green-100 text-green-700 shrink-0">
                        Approved
                    </span>
                </div>
    ...
    <div class="w-full max-w-xs md:max-w-7xl mx-auto mt-8">
        <h3 class="text-lg font-semibold text-gray-800 mb-3">Past</h3>
        ...
        @forelse($past as $s)
            @php
                ...
                // Show "Done" for approved sessions that are in the past
                $displayStatus = ($status === 'approved') ? 'done' : $status;

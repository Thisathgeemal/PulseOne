    ...
    <div class="w-full max-w-xs md:max-w-7xl p-8 bg-white rounded-lg my-4 text-center shadow-md mx-auto animate-fade-in"
        ...
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <div class="flex justify-between items-center space-x-3 sm:space-x-4">
                <div
                    class="relative w-full sm:w-auto flex flex-col sm:flex-row sm:items-center sm:space-x-4 space-y-0 sm:space-y-0">
                    <form method="GET" action="{{ route('member.membership') }}"
                        ...
                        {{-- Global Search Input --}}
                        <div class="relative">
                            <input type="text" id="searchBar" name="search" placeholder="Search Memberships"
                ...
                <div class="relative group">
                    <form method="POST" action="{{ route('member.membership.report') }}" target="_blank"
                        class="relative group" onsubmit="setCurrentDateTime()">
                        @csrf
                    ...
                    <tbody>
                        @foreach ($memberships as $index => $membership)
                            <tr class="hover:bg-gray-200 transition duration-200">
                                ...
                                <td class="py-3 px-4 border-b border-gray-200">{{ $membership->membershipType->type_name }}
                                </td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">
                                    {{ \Carbon\Carbon::parse($membership->start_date)->format('Y-m-d') }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">
                                    {{ \Carbon\Carbon::parse($membership->end_date)->format('Y-m-d') }}</td>
                                <td class="py-3 text-left px-4 border-b border-gray-200">
                                    <span
        ...
        <!-- Modal -->
        <div id="addMembershipModel" role="dialog" aria-modal="true"
            ...
            <div class="bg-white rounded-lg p-6 w-full max-w-md shadow-[0_0_15px_4px_rgba(241,30,19,0.5)]">
                ...
                <div id="membership-section" class="form-section">
                    ...
                    <form method="POST" id="membershipForm">
                        ...
                        <div class="flex justify-end space-x-2 mt-5">
                            ...
                            <button type="button" class="btn-primary px-4 py-2 rounded"
                ...
                <div id="payment-section" class="form-section hidden">
                    ...
                    <form method="POST" class="space-y-6" action="{{ route('member.membership.buy') }}">
                        ...
                        <div>
                            ...
                            <select id="card_type" name="card_type" required autofocus
                                ...
                                <option value="mastercard" {{ old('card_type') == 'mastercard' ? 'selected' : '' }}>
                                    MasterCard</option>
                                <option value="amex" {{ old('card_type') == 'amex' ? 'selected' : '' }}>American Express
                                </option>
                                <option value="discover" {{ old('card_type') == 'discover' ? 'selected' : '' }}>Discover
                                </option>
                            </select>
                            @error('card_type')
                            ...
                            <div>
                                ...
                                <select id="expiry_year" name="expiry_year" required autofocus
                                    ...
                                    @for ($y = date('Y'); $y <= date('Y') + 15; $y++)
                                        <option value="{{ $y }}"
                                            {{ old('expiry_year') == $y ? 'selected' : '' }}>{{ $y }}</option>
                                    @endfor
                                </select>
                                @error('expiry_year')
                                    <span class="text-red-600 text-sm">{{ $message }}</span>
                        ...
                        <div class="flex justify-end space-x-2 mt-5">
                            <button type="button" class="bg-gray-300 hover:bg-gray-400 text-black px-4 py-2 rounded"
                                onclick="backStep()">Previous</button>
                            <button type="submit" class="btn-primary px-4 py-2 rounded">Buy
                                Now</button>
                        </div>
                    </form>
                </div>

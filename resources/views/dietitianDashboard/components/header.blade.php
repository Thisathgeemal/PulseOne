    ...
    <div x-show="showProfile" x-transition
        ...
        <div class="p-8">
            ...
            <div class="relative text-center mb-4">
                ...
                <form action="{{ route('dietitian.profile.removeImage') }}" method="POST" class="mt-2">
                    @csrf
                    @method('DELETE')
            ...
            <!-- Profile Form -->
            <form method="POST" action="{{ route('dietitian.profile.update') }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
...
@push('scripts')
    ...
    <script>
        document.getElementById('verifyPasswordBtn')?.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch("{{ route('dietitian.profile.checkPassword') }}", {
                    method: 'POST',
                    headers: {

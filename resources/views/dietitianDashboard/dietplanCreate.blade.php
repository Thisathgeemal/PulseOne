                ...
                <div class="mb-6">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-80 overflow-y-auto border rounded-lg p-4"
                            id="availableMeals">
                            @foreach ($meals as $meal)
                                <div class="meal-item relative border rounded-lg p-3 cursor-pointer hover:bg-gray-50 hover:border-red-300"
                                    data-meal-id="{{ $meal->meal_id }}" data-meal-name="{{ $meal->description }}"
                                    data-calories="{{ $meal->calories_per_serving }}"
                                    data-protein="{{ $meal->protein_grams }}" data-carbs="{{ $meal->carbs_grams }}"
                                    data-fat="{{ $meal->fats_grams }}">
                                    ...
                                    <span
                                        class="selected-badge hidden absolute top-2 right-2 w-6 h-6 bg-green-600 text-white rounded-full flex items-center justify-center text-xs font-bold">
                                        ✓
                                    </span>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function addMealToSelection(mealElement) {
                ...
                // Mark the meal card as selected (show badge)
                const badge = mealElement.querySelector('.selected-badge');
            ...
            function removeMealFromSelection(mealId) {
                selectedMeals[currentMealType] = selectedMeals[currentMealType].filter(meal => meal.id !== mealId);
                // remove selected badge from the meal card if present
                const mealCard = document.querySelector(`.meal-item[data-meal-id="${mealId}"]`);
                if (mealCard) {
                    const badge = mealCard.querySelector('.selected-badge');

...
@section('content')
    ...
    <div class="grid gap-6 sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-2">
                    ...
                    <div class="flex items-start gap-4">
                        ...
                        <div class="text-left">
                            ...
                            <p class="text-sm text-gray-600">
                                ...
                                {{ ucwords(str_replace('_', ' ', $req->goal)) }}
                            </p>
                            <p class="text-sm text-gray-600 mt-1">
                                <span class="font-semibold text-gray-700">Timeframe:</span>
                                {{ ucwords(str_replace('_', ' ', $req->timeframe)) }}
                            </p>
                        </div>
                    </div>
                ...
                <div class="mt-3 flex gap-3 justify-end opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    @if ($req->status == 'Pending')
                        <form method="POST" action="{{ route('dietitian.request.update', $req->request_id) }}">
                            ...
                            <button type="submit"
                                class="w-[110px] px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-sm font-medium rounded-lg transition text-center">
                                Approve
                            </button>
                        </form>
                        <form method="POST" action="{{ route('dietitian.request.update', $req->request_id) }}">
                            ...
                            <button type="submit"
                                class="w-[110px] px-4 py-2 bg-red-500 hover:bg-red-600 text-white text-sm font-medium rounded-lg transition text-center">
                                Reject
                            </button>
                        </form>
                    @elseif($req->status == 'Approved')
                        <a href="{{ route('dietitian.dietplan.create', $req->request_id) }}"
        ...
        @empty
            <div
                class="text-center text-gray-500 text-lg font-medium col-span-full bg-white rounded-xl border border-gray-200 shadow-md p-6 mt-3">

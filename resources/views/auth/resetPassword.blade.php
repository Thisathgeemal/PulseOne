    ...
    <section class="h-[calc(100vh-64px)] bg-cover bg-center flex items-center justify-center min-h-full" style="background-image: url('{{ url('images/gym-red.jpg') }}');">
        <div class="w-full max-w-md py-10 px-6 bg-white rounded-lg shadow-md">
            ...
            <form method="POST" action="{{ route('password.update') }}" class="flex flex-col gap-6">
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 items-center gap-2 mb-2">
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-gray-700 items-center gap-2">
                        <i class="fa-solid fa-lock text-sm mr-1.5 ml-1"></i>
                        Confirm Password
                    </label>
    ...
    @if (session('reset_error'))
        <script>
            Swal.fire({
                ...
                confirmButtonColor: '#dc2626',
                confirmButtonText: 'OK',
                text: 'This password reset token is invalid or has already been used',
            });
        </script>
    @endif

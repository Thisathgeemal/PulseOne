...
class TrainerProfileTest extends DuskTestCase
{
    protected $trainer;
    ...
    protected function setUp(): void
    {
        ...
        User::where('email', 'trainer@test.com')->delete();

        $role = Role::firstOrCreate(['role_name' => 'Trainer']);
    ...
    public function test_trainer_can_update_profile()
    {
        $this->browse(function (Browser $browser) {
            ...
            $browser->visit('/login')
                ...
                ->press('Sign in')
                ->waitForText('Welcome, ' . $this->trainer->first_name);
    ...
    public function test_trainer_can_change_password()
    {
        $this->browse(function (Browser $browser) {
            $browser->visit('/login')
                ->type('email', 'trainer@test.com')
            ...
            $user = User::find($this->trainer->id);
            $this->assertTrue(Hash::check('newpassword123@12', $user->password));
        });
    }

    public function test_trainer_can_update_profile_image()
    {
        $this->browse(function (Browser $browser) {
            $browser->visit('/login')
                ->type('email', 'trainer@test.com')
            ...
            $user = User::find($this->trainer->id)->fresh();
    ...
    public function test_trainer_can_remove_profile_image()
    {
        $this->browse(function (Browser $browser) {
            $browser->visit('/login')
                ->type('email', 'trainer@test.com')
    ...
    public function test_trainer_can_toggle_mfa()
    {
        $this->browse(function (Browser $browser) {
            $browser->visit('/login')
                ->type('email', 'trainer@test.com')
            ...
            $this->assertTrue($this->trainer->mfa_enabled);
        });
    }

    public function test_trainer_can_logout_specific_device()
    {
        $this->browse(function (Browser $browser) {
    ...
    public function test_trainer_can_logout_all_other_devices()
    {
        $this->browse(function (Browser $browser) {
